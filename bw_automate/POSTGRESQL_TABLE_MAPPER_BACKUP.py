    ...
    def _initialize_orm_patterns(self) -> List[Dict]:
        ...
        return [
            ...
            # Tortoise ORM
            {
                'pattern': r'table\s*=\s*["\'](\w+)["\']',
                'table_group': 1,
                'context': 'tortoise_model',
                'confidence': 0.90
            },
            # Pony ORM
            {
                'pattern': r'_table_\s*=\s*["\'](\w+)["\']',
                'table_group': 1,
                'context': 'pony_model',
                'confidence': 0.90
            },
            # Pandas operations
            {
                'pattern': r'(read_sql|read_sql_table|read_sql_query|to_sql)\s*\(\s*["\']([^"\']*)["\']',
                'table_group': 2,
                'context': 'pandas_operation',
                'confidence': 0.85
            },
            {
                'pattern': r'(read_sql_table|to_sql)\s*\([^,]*["\'](\w+)["\']',
                'table_group': 2,
                'context': 'pandas_table',
                'confidence': 0.90
            },
            # Migration operations
            {
                'pattern': r'(create_table|drop_table|rename_table)\s*\(\s*["\'](\w+)["\']',
                'table_group': 2,
                'context': 'migration_operation',
                'confidence': 0.95
            },
            # op. operations (Alembic)
            {
                'pattern': r'op\.(create_table|drop_table|rename_table)\s*\(\s*["\'](\w+)["\']',
                'table_group': 2,
                'context': 'alembic_operation',
                'confidence': 0.95
            },
            # Database connections and cursors
            {
                'pattern': r'(copy_from|copy_to|copy_expert)\s*\([^,]*["\'](\w+)["\']',
                'table_group': 2,
                'context': 'copy_operation',
                'confidence': 0.90
            },
